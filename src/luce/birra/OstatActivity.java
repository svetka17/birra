...
public class OstatActivity extends FragmentActivity implements LoaderCallbacks<Cursor> {
    ...
    btnAdd.setOnClickListener(new OnClickListener() {
        public void onClick(View v) {
        	MainActivity.excel(OstatActivity.this, OstatActivity.this, "","", 
        			tvIdPgr.getText().toString(), "Наличие в магазине", 5);
        }
      });
    					   ...
    					   if (cc.moveToFirst()) { 
    					        do {countT=
    					        		...
    					        		(cc.getInt(cc.getColumnIndex("id_tmc")),cc.getInt(cc.getColumnIndex("keg")), 
    					        				cc.getDouble(cc.getColumnIndex("kol")), (cc.getDouble(cc.getColumnIndex("kol"))>0?cc.getDouble(cc.getColumnIndex("kol")):0), 0, 0, 0, /*cc.getDouble(cc.getColumnIndex("kol")),*/0, cc.getInt(cc.getColumnIndex("ed")), 0,0, cc.getInt(cc.getColumnIndex("id_post")), 0, "ОБНУЛЕНИЕ ОСТАТКА ID="+id, MainActivity.getIntDataTime(), 5);
    				    					   ...
    				    					   if (cc.moveToFirst()) { 
    				    					        do {countT=
    				    					        		MainActivity.db.addRecRASXODcount(cc.getInt(cc.getColumnIndex("id_tmc")), cc.getInt(cc.getColumnIndex("keg")),cc.getDouble(cc.getColumnIndex("kol"))-k,
    				    					        				...
    				    					        				0,cc.getInt(cc.getColumnIndex("ed")), 0,0, cc.getInt(cc.getColumnIndex("id_post")), 0, "ИЗМЕНЕНИЕ ОСТАТКА РАСХОД ID="+idd, MainActivity.getIntDataTime(), 5);
  ...
  static class MyCursorLoader extends CursorLoader {
    ...
    public Cursor loadInBackground() {
    			...
    			+ "on O.id_tmc=R.id_tmc and O.id_post=R.id_post and O.ed=R.ed and O.keg=R.keg "
    			+ "where "+(cbVis.isChecked()?" O.kol!=0 ":" O.kol=O.kol ")+((Integer.parseInt(tvIdPgr.getText().toString())==0)?"":" and T.pgr="+tvIdPgr.getText().toString())  
